...
- `charts/marketplace`, `charts/storefront`, `charts/agent` va `charts/motiv-agent` production Helm source-of-truth hisoblanadi. Chartlar brendsiz: brend — bu values fayli, chart emas.
- Image taglari application repolaridagi `helm/<brend>-values.yaml` fayllarida saqlanadi (masalan `backend/helm/lattaputta-values.yaml`, `backend/helm/venu-values.yaml`). Har bir application CD o'z repo values faylini standart `GITHUB_TOKEN` bilan yangilaydi.
- CD teg yangilash sikli `helm/*values.yaml` globiga tayanadi — values faylini nomlashda `-values.yaml` qo'shimchasini saqlang, aks holda teg jim turib yangilanmaydi.
...
- Namespace'lar Argo destination orqali aniq, lekin nomlar tarixiy: `venu` namespace'ida **Latta Putta**, `venu-market`da esa **Venu** ishlaydi.
